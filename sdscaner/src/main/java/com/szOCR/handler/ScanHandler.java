...
public final class ScanHandler extends Handler {
	...
	public void handleMessage(Message message) {
		if (message.what == R.id.auto_focus) {// Log.d(TAG, "Got auto-focus message");
			...
			Bitmap bmImage = bundle == null ? null : (Bitmap) bundle
					.getParcelable(CGlobal.PHONENUMBER_BITMAP);//
			mActivity.returnRecogedData((RecogResult) message.obj, bmImage);//

		} else if (message.what == R.id.recog_failed) {// We're decoding as fast as possible, so when one decode fails,
			// start another.
			state = State.PREVIEW;
			mCameraPreview.requestPreviewFrame(decodeThread.getHandler(),
					R.id.recog_start);
