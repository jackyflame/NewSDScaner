...
public final class RecogHandler extends Handler {
	...
	RecogHandler(ScanActivity activity) {
	...
	private void decode(byte[] data, int width, int height) {
		...
		Rect orgRect = CGlobal.getOrgCropRect(width, height, rot,
				CGlobal.g_rectCrop);
		RecogResult rawResult = CGlobal.myEngine.RecogPhoneNumber_data(data,
				width, height, rot, orgRect);
		long end = System.currentTimeMillis();
		rawResult.m_nRecogTime = (end - start);
		if (rawResult.m_nResultCount > 0) {

			Log.d("RecogResult - ", rawResult.m_szNumber.toString());
			Bitmap recogBitmap = CGlobal.makeCropedGrayBitmap(data, width,
					height, rot, orgRect);
			Message message = Message.obtain(mActivity.getHandler(),
					R.id.recog_succeeded, rawResult);
